...
import type { EmittedAsset, NormalizedInputOptions, Plugin } from 'rollup'
...
function mapEntry(entryName: string, entry: SvgSpriteEntry): Promise<{ asset: EmittedAsset; files: string[] }> {
  ...
  return new Promise((resolve, reject) => {
    glob(entry.input, { ignore: entry.except }, (exception, files) => {
      if (exception)
        reject(exception)
...
export default function createSvgSprite(options: SvgSpriteOptions): Plugin {
  return {
    ...
    async buildStart(_: NormalizedInputOptions): Promise<void> {
      ...
      const entries = await Promise.all(Object.entries(options.entry).map(([entryName, entry]) => mapEntry(entryName, entry)))
